...
class TimmBackbone(nn.Module):
    def __init__(self, name, return_interm_layers, main_layer=-1, group_norm=False):
        super().__init__()
        # Offline-first: avoid downloading timm weights by default.
        # Use env MDETR_TIMM_PRETRAINED=1 to allow pretrained, and MDETR_TIMM_CKPT to load a local checkpoint.
        import os, torch
        use_pretrained = os.environ.get("MDETR_TIMM_PRETRAINED", "0") in ("1", "true", "True")
        ...
        # Optionally load a local checkpoint to avoid network calls
        local_ckpt = os.environ.get("MDETR_TIMM_CKPT", "")
        if local_ckpt and os.path.isfile(local_ckpt):
            try:
                state = torch.load(local_ckpt, map_location="cpu")
                if isinstance(state, dict) and "state_dict" in state:
                    state = state["state_dict"]
                # Some checkpoints may be under "model" key
                if isinstance(state, dict) and "model" in state and isinstance(state["model"], dict):
                    state = state["model"]
                missing_keys, unexpected_keys = backbone.load_state_dict(state, strict=False)
            except Exception:
                # Silently continue if local load fails to avoid breaking initialization
                pass
