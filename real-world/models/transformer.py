...
class Transformer(nn.Module):
    def __init__(
        ...
        # Offline-first: allow specifying a local directory for the text encoder/tokenizer.
        # If TRANSFORMERS_OFFLINE is set or a local dir is provided, prefer local files only.
        local_text_dir = os.environ.get("MDETR_TEXT_ENCODER_DIR", "").strip()
        offline = os.environ.get("TRANSFORMERS_OFFLINE", "0") in ("1", "true", "True")
        if local_text_dir and os.path.isdir(local_text_dir):
            self.tokenizer = RobertaTokenizerFast.from_pretrained(local_text_dir, local_files_only=True)
            self.text_encoder = RobertaModel.from_pretrained(local_text_dir, local_files_only=True)
        else:
            # Fallback to model id; if offline, this will use cache only
            self.tokenizer = RobertaTokenizerFast.from_pretrained(
                text_encoder_type, local_files_only=offline
            )
