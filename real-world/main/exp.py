...
def run_reasoning(folder_name, llm_prompter, global_sg):
    with open(f'main/thor_tasks/{folder_name}/task.json') as f:
        task = json.load(f)
    ...
    with open('main/state_summary/{}/state_summary_L2.txt'.format(folder_name), 'r') as f:
        L2_captions = f.readlines()

    # Load L1 captions from state_summary_L1.txt
    with open('main/state_summary/{}/state_summary_L1.txt'.format(folder_name), 'r') as f:
        L1_captions = f.readlines()
